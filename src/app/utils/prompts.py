...
用户目标：{user_command}
网格信息：{grid_size}x{grid_size}, cell={cell_width:.0f}x{cell_height:.0f}

{skill_catalog}

请基于截图输出 JSON 计划（数组）。
动作仅可使用：open_search / click_grid / input_text / press_key / wait / done
- 最后一步必须 done
- 不确定时优先 wait
- click_grid 不得固定编号
{skill_guidance}
...
必须用 function calling，每轮只调一个函数：click_region / press_key / paste_content
- 不确定时优先 wait
- 目标完成时输出 done
{skill_guidance}
"""
