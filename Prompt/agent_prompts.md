...
你已经拿到当前截图（带红色网格编号），请基于该截图直接生成“完整执行计划”。

网格信息：
- 网格大小：{grid_size}x{grid_size}
- 单元格尺寸：{cell_width:.0f}x{cell_height:.0f}
...
2. click_grid
   {{"action":"click_grid","grid":17,"offset_ratio":0.8,"reason":"点击目标元素"}}
   - grid 从 1 开始
   - offset_ratio 可选，默认 0.5（中心）；0.2 更偏上，0.8 更偏下
...
- 最后一步必须是 done。
- click_grid 不能固定使用某个编号，必须依据当前截图判断。
...
可用函数：click_region / press_key / paste_content

规则：
- 不要输出 JSON 文本，不要输出 Markdown，不要输出解释文本。
- 不确定时优先保守，不要乱点。
- 如果目标已完成，请输出 done。
- click_region 必须基于当前截图判断，不要固定使用 8。
