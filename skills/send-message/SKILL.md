...
1. Open global search (`command+k` on macOS / `ctrl+k` on Windows).
2. Input the contact name in search.
3. Click the correct contact from search results (exact contact match only).
4. After contact click succeeds, interact with the chat window. **CRITICAL: Act as a smart human assistant! If the user's `message` intent is "发个表情包", you can use `click_position` to click the emoji button on the screen.**
...
- If uncertain whether chat has switched to the target contact, prefer a conservative action (`wait` or re-locate) over sending.
